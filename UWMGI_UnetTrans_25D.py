...
ADJACENT_SLICES = 2
preprocessed_folder = f'./preprocessed_384x384_{ADJACENT_SLICES}_25d'
...
class DataLoader(Sequence):
    ...
    def __getitem__(self, index):
        ...
        X = np.empty((len(indices), *IMAGE_SIZE, ADJACENT_SLICES * 2 + 1), dtype='float32')
...
for fold in range(1, KFOLD + 1):
    ...
    model = sm.Unet(MODEL_NAME, input_shape=(None, None, ADJACENT_SLICES * 2 + 1), classes=3, activation='sigmoid', encoder_weights=None)
