...
'''
def Dice_Coef(spartial_axis = (1,2), mean_axis=(1, 0), smooth=1e-6):
...
def dice_loss(spartial_axis=(1,2,3), mean_axis=(1, 0), smooth = 1e-6):
    def dice_loss(y_true, y_pred):
        ...
        numerator = 2 * tp + smooth
        denominator = 2 * tp + fn + fp + smooth
        return tf.math.reduce_mean(1 - numerator / denominator, mean_axis) # Average over classes
    return dice_loss

def bce_dice_loss(spartial_axis=(1,2,3), mean_axis=(1, 0),  smooth = 1e-6):
    dice_loss_func = dice_loss(spartial_axis, mean_axis, smooth)
